# models_test.py - Modelos compatíveis com SQLite para testes
...
class Research(models.Model):
    ...
    # Usar TextField com JSON para compatibilidade com SQLite
    keywords_json = models.TextField(default='[]', blank=True)
    members_json = models.TextField(default='[]', blank=True)
    campus = models.CharField(max_length=100)
    ...
    def members(self, value):
        if isinstance(value, list):
            ...
            self.members_json = json.dumps([])

    def save(self, *args, **kwargs):
        # Garantir que os campos JSON estão inicializados
        if not self.keywords_json:
            self.keywords_json = '[]'
        if not self.members_json:
            self.members_json = '[]'
